...
static std::string fistPageChar ("[0]");
static std::vector<bool> firstPages;
...
///////////////////////////////////////////////////////////////////////////
{
    ...
    for (i = 1; i < argc - 1; i++) {
        ...
        std::size_t found = currFile.find(fistPageChar);

        if (found!=std::string::npos) {
            currFile = currFile.substr(0, found);
            std::cout<<"currFile:"<<currFile<<"firstPage"<<"True"<<std::endl;
            firstPages.push_back(true);
        } else {
            firstPages.push_back(false);
        }

        std::unique_ptr<PDFDoc> doc = std::make_unique<PDFDoc>(std::make_unique<GooString>(currFile));
    ...
    for (i = 0; i < (int)docs.size(); i++) {
        for (j = 1; j <= docs[i]->getNumPages(); j++) {
            ...
            // cout<<"Annots:"<<"Type:"<<annotsObj.getType()<<"Null:"<<annotsObj.isNull()<<endl;
