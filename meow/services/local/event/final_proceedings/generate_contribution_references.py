...
from meow.tasks.local.reference.models import (
    ContributionRef,
    ReferenceStatus,
    Reference,
)
...
from meow.utils.datetime import (
    ...
    format_datetime_month_name,
    format_datetime_year_num,
)
...
class JinjaXMLBuilder:
    ...
    async def build_reference_xml(self, contribution: ContributionRef) -> str:
        return await self.env.get_template("reference.xml.jinja").render_async(
            contribution.as_dict()
        )


async def generate_contribution_references(
        ...
        await res.send(
            {
                ...
                "value": await build_contribution_reference(
                    event,
                    contribution,
                    xml_builder,
                    xslt_functions,
...
async def build_contribution_reference(
    ...
    contribution_ref: ContributionRef = None
...
async def contribution_data_factory(
    ...
    number_of_pages = (
        contribution.metadata.get("page_count", 0) if contribution.metadata else 0
    )

    location: str = settings.get("location", "")

    conference_code = re.sub(r"[^a-zA-Z]", "", event.name).lower() + str(
        event.start.year
    )
    ...
    return ContributionRef(
        ...
        authors_list=list(contribution.authors_list),
...
def refill_contribution_reference(
