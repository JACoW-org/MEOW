...
async def build_hep_payloads(proceedings_data: ProceedingsData) -> ProceedingsData:
    ...
    logger.info("build_hep_payloads - " + f"contributions: {total_contributions}")
    ...
    async with create_task_group() as tg:
        # conference DOI
        tg.start_soon(
            generate_conference_hep_payload_task,
            capacity_limiter,
            proceedings_data,
            hep_dir,
        )
...
async def concat_hep_files(
    proceedings_data: ProceedingsData, hep_dir: Path, hep_file: Path
):
    hep_event_path = [Path(hep_dir, f"{proceedings_data.event.id}.json")]
...
async def generate_conference_hep_payload_task(
    capacity_limiter: CapacityLimiter, proceedings_data: ProceedingsData, hep_dir: Path
) -> None:
    """ """
...
async def generate_hep_payload_task(
    capacity_limiter: CapacityLimiter, contribution_doi: ContributionDOI, hep_dir: Path
) -> None:
    """ """
