...
async def download_event_materials(
...
async def file_download_task(
    ...
    async with capacity_limiter:
        try:
            ...
            indico_cookies = dict(
                indico_session_http=http_sess, indico_session=https_sess
            )
            ...
            if await is_to_download(pdf_file, pdf_md5):
                ...
                await download_file(url=pdf_url, file=pdf_file, cookies=indico_cookies)
            # else:
            #     logger.info(f"cached_file --> {pdf_url}")

            await res.send(
                {"index": current_index, "total": total_files, "file": current_file}
            )
