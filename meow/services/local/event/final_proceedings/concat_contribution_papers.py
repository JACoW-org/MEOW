...
async def concat_contribution_papers(
    ...
    files_data = await extract_proceedings_papers(
        proceedings_data,
        callback,
    )

    if len(files_data) > 0:
        await brief_pdf_task(
            proceedings_data,
            files_data,
            cache_dir,
            doi_conf,
...
async def brief_pdf_task(
    proceedings_data: ProceedingsData,
    files_data: list[FileData],
    cache_dir: Path,
    doi_conference: str,
    absolute_pdf_link: bool,
):
    ...
    brief_pdf_links = (
        [f"https://jacow.org/{doi_conference}/pdf/{f.filename}" for f in files_data]
        if absolute_pdf_link
        else [f.filename for f in files_data]
    )
    ...
    async with create_task_group() as tg:
        for index, vol_pdf_files_chunk in enumerate(split_list(pdf_files, chunk_size)):
            tg.start_soon(
                concat_chunks,
                f"{brief_pdf_chunk_path}." + "{:06d}".format(index),
                vol_pdf_files_chunk,
                brief_pdf_results,
                True,
                capacity_limiter,
    ...
    pdf_parts = (
        [str(brief_pre_pdf_path)] + brief_pdf_results
        if brief_pre_pdf_path
    ...
    try:
        if (
            await brief_links(
                str(brief_pdf_chunk_path), str(brief_pdf_links_path), brief_pdf_links
            )
            != 0
        ):
...
async def vol_pdf_task(
    ...
    vol_pre_pdf_path = await get_vol_pre_pdf_path(
        proceedings_data,
        cache_dir,
    )

    [vol_toc_pdf_path, vol_toc_links_path] = await get_vol_toc_pdf_path(
        proceedings_data,
        vol_pre_pdf_path,
        cache_dir,
        callback,
    ...
    async with create_task_group() as tg:
        for index, pdf_files_chunk in enumerate(split_list(pdf_files, chunk_size)):
            tg.start_soon(
                concat_chunks,
                f"{vol_pdf_chunk_path}." + "{:06d}".format(index),
                pdf_files_chunk,
                vol_pdf_results,
                False,
                capacity_limiter,
    ...
    try:
        if (
            await vol_toc_links(
                str(vol_pdf_chunk_path),
                str(vol_pdf_links_path),
                str(vol_toc_links_path),
            )
            != 0
        ):
...
async def get_vol_toc_pdf_path(
    proceedings_data: ProceedingsData,
    vol_pre_pdf_path: Path | None,
    cache_dir: Path,
    callback: Callable,
    toc_grouping: list[str],
):
    ...
    try:
        for session in proceedings_data.sessions:
            session_ids[session.id] = dict(
                session_data=session, contributions=[], page=0
            )
        ...
        if (
            await vol_toc_pdf(
                str(vol_toc_pdf_path),
                str(vol_toc_links_path),
                str(vol_toc_conf_path),
            )
            != 0
        ):
