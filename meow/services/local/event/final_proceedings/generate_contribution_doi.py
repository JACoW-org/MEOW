...
from meow.tasks.local.doi.utils import (
    generate_doi_external_label,
    generate_doi_external_url,
    generate_doi_identifier,
    generate_doi_internal_url,
        ...
        async with create_task_group() as tg:
            async with send_stream:
                for contribution_data in contributions:
                    tg.start_soon(
                        generate_contribution_doi_task,
                        capacity_limiter,
                        proceedings_data.event,
                        contribution_data,
                        settings,
                        config,
                        send_stream.clone(),
...
async def generate_conference_hep_task(
    ...
    conference_hep = {
        "titles": [{"source": "JACOW", "title": settings.get("booktitle_long", "")}],
        "imprints": [{"date": format_datetime_dashed(datetime_now(event_timezone))}],
        "publication_info": [
            {
        ...
        "isbns": [{"value": settings.get("isbn", "")}],
        "document_type": ["proceedings"],
        "urls": [{"value": doi_landing_page.lower()}],
        "license": [
            {
                "imposing": "JACOW",
                "license": "CC-BY-4.0",
        ...
        "authors": [
            {
                ...
                "raw_affiliations": [
                    {"value": affiliation} for affiliation in editor.affiliations
                ],
            }
...
async def generate_conference_doi_task(
    ...
    doi_url: str = (
        ...
        if config.generate_external_doi_url
        else generate_doi_internal_url(
            organization=settings.get("doi_organization", "JACoW"),
            conference=settings.get("doi_conference", "CONF-YY"),
        )
    )
    ...
    conference_doi = {
        ...
        "titles": [{"title": settings.get("booktitle_long", ""), "lang": "en-us"}],
...
async def generate_contribution_doi_task(
...
async def build_contribution_doi(
    event: EventData,
    contribution: ContributionData,
    ...
    doi_data = ContributionDOI(
        ...
        revisitation_date=format_datetime_doi(contribution.revisitation)
        ...
        if contribution.page and contribution.metadata
        else "",
        num_of_pages=contribution.metadata.get("page_count", 0)
...
def refill_contribution_doi(
    proceedings_data: ProceedingsData, results: dict, callable: Callable
) -> ProceedingsData:
    start_page: int = 0
    ...
    for contribution_data in proceedings_data.contributions:
        if callable(contribution_data):
            ...
            # logger.info(f"code={code} doi_data")
