...
from meow.utils.collections import (
    get_authors_initials_dict,
...
class JinjaTemplateRenderer:
    ...
    async def render(
        ...
        html = await self.env.get_template(template).render_async(params)

        html = (
            html
            if minify is False
            else minify_html.minify(
                html, remove_processing_instructions=True, keep_comments=True
            )
        )

        html = escape_special_characters(html) if escape else html

        return html

    async def render_config_toml(
        self,
        event: EventData,
        logo: MaterialData | None,
        poster: MaterialData | None,
        volumes: list[MaterialData],
        attachments: list[MaterialData],
        settings: dict,
    ) -> str:
    ...
    async def render_contribution_partial(
        self,
        contribution: ContributionData,
        include_event_slides: bool,
        include_event_posters: bool,
        is_published: bool,
    ) -> str:
        return await self.render(
            "contribution_partial.html.jinja",
            minify=True,
            escape=True,
            params=dict(
                contribution=contribution.as_dict(),
                include_event_slides=include_event_slides,
                include_event_posters=include_event_posters,
    ...
    async def render_session_partial(self, sessions: list[SessionData]) -> str:
        return await self.render(
            "session_partial.html.jinja",
            minify=True,
            params=dict(sessions=[s.as_dict() for s in sessions]),
        )
    ...
    async def render_classification_partial(
        self, classifications: list[TrackData]
    ) -> str:
        return await self.render(
            "classification_partial.html.jinja",
            minify=True,
            params=dict(classifications=[s.as_dict() for s in classifications]),
        )

    async def render_classification_page(
        self,
        event: EventData,
        classification: TrackData,
        contributions: list[ContributionData],
    ) -> str:
        return await self.render(
            "classification_page.html.jinja",
            minify=False,
            params=dict(
                event=event.as_dict(),
    ...
    async def render_institute_partial(
        self, institutes: list[AffiliationData], authorsGroups: dict[str, list[dict]]
    ) -> str:
        return await self.render(
            "institute_partial.html.jinja",
            minify=True,
            params=dict(
    ...
    async def render_institute_page(
        self,
        event: EventData,
    ...
    async def render_doi_per_institute_partial(
        self,
    ...
    async def render_doi_per_institute_page(
        self,
        event: EventData,
        institute: AffiliationData,
        contribution: ContributionData,
        ...
        return await self.render(
            "doi_per_institute_page.html.jinja",
            minify=False,
            params=dict(
                event=event.as_dict(),
                institute=institute.as_dict(),
    ...
    async def render_doi_list(
        self, event: EventData, contributions: list[ContributionData]
    ):
        return await self.render(
            "doi_list.html.jinja",
            minify=False,
    ...
    async def render_doi_partial(
        self, event: EventData, contribution: ContributionDOI
    ) -> str:
        return await self.render(
            "doi_partial.html.jinja",
            minify=True,
            params=dict(event=event.as_dict(), contribution=contribution.as_dict()),
        )
