...
async def copy_references_jsonl(
    ...
    references_name = f"{proceedings_data.event.id}_refs"
    references_dir = Path("var", "run", f"{references_name}")
    ...
    json_src = Path(references_dir, "references.jsonl")
    json_dest = Path(json_dest_dir, f"references-{json_dest_name}.jsonl")

    await json_dest.unlink(missing_ok=True)
    await copy(str(json_src), str(json_dest))


async def copy_inspirehep_jsonl(
    proceedings_data: ProceedingsData, cookies: dict, settings: dict
):
    html_dir_name = f"{proceedings_data.event.id}_src"
    html_base_dir: Path = Path("var", "run", html_dir_name)
    ...
    partials = [
        {
            ...
            "src": Path("layouts", "partials", "author", "list.html"),
        },
        {
            "name": "classification_list.html",
            "src": Path("layouts", "partials", "classification", "list.html"),
        },
        {
            "name": "doi_per_institute_list.html",
            "src": Path("layouts", "partials", "doi_per_institute", "list.html"),
        },
        {
            "name": "institute_list.html",
            "src": Path("layouts", "partials", "institute", "list.html"),
        },
        {
            "name": "keyword_list.html",
            "src": Path("layouts", "partials", "keyword", "list.html"),
        },
        {
            "name": "session_list.html",
            "src": Path("layouts", "partials", "session", "list.html"),
        },
    ]
