...
async def event_doi_delete(
    ...
    try:
        ...
        async with acquire_lock(event_id) as lock:
            ...
            async for r in _event_doi_delete(
                event,
                cookies,
                settings,
                lock,
...
async def _event_doi_delete(
    ...
    yield dict(
        type="progress",
        value=dict(phase="delete_contribution_doi", text="Delete contribution doi"),
    )
