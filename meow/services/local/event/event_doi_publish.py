...
from meow.services.local.event.doi.event_doi_publish import publish_contribution_doi


logger = lg.getLogger(__name__)


async def event_doi_publish(event: dict, cookies: dict, settings: dict) -> AsyncGenerator:
    """ """
    ...
    try:
        ...
        async with acquire_lock(event_id) as lock:
            ...
            async for r in _event_doi_publish(event, cookies, settings, lock):
                yield r
...
async def _event_doi_publish(event: dict, cookies: dict, settings: dict, lock: RedisLock) -> AsyncGenerator:
    """ """

    logger.info('event_doi_publish - create_final_proceedings')
