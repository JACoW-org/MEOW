...
def contribution_editable_factory(
    editable: Any, event_timezone: str
) -> EditableData | None:
    if not editable:
        return None
    ...
    latest_revision = (
        contribution_revision_factory(
            editable.get("latest_revision", None), event_timezone
        )
        if editable.get("latest_revision", None)
        else None
    )
...
def contribution_data_factory(
    ...
    slides_editable: Any = find(
        contrib_editables,
        lambda x: x.get("type", 0) == EditableData.EditableType.slides,
    )
    ...
    revisitation_revision = (
        revisitation_revisions[0] if len(revisitation_revisions) else None
    )
    ...
    reception = reception_revision.creation_date if reception_revision else None

    revisitation = (
        revisitation_revision.creation_date if revisitation_revision else None
    )

    acceptance = acceptance_revision.qa_approved_date if acceptance_revision else None
    ...
    authors = [
        ...
        for person in (
            contribution.get("primary_authors", []) + contribution.get("coauthors", [])
        )
    ]
    ...
    contribution_data = ContributionData(
        ...
        track=track_data_factory(contribution.get("track")),
        keywords=list(set([])),
        ...
        start=datetime_localize(
            datedict_to_tz_datetime(contribution.get("start_dt")), event_timezone
        ),
        ...
        issuance=issuance,
        speakers=[
            event_person_factory(person) for person in contribution.get("speakers", [])
        ],
        primary_authors=[
            event_person_factory(person)
        ...
        coauthors=[
            event_person_factory(person) for person in contribution.get("coauthors", [])
        ],
        editors=editors,
    )
...
def contribution_revision_factory(revision: Any, event_timezone: str) -> RevisionData:
    return RevisionData(
        ...
        creation_date=datetime_localize(
            datedict_to_tz_datetime(revision.get("created_dt")), event_timezone
        ),
        files=[contribution_file_factory(file) for file in revision.get("files", [])],
        tags=[contribution_tag_factory(tag) for tag in revision.get("tags", [])],
    )


def contribution_revision_comment_factory(
    comment: Any, event_timezone: str
) -> RevisionCommentData:
    return RevisionCommentData(
        ...
        created_dt=datetime_localize(
            datedict_to_tz_datetime(comment.get("created_dt")), event_timezone
        ),
...
def contribution_file_factory(file: Any) -> FileData:
    file_type: int = (
        file.get("file_type", None).get("type") if file.get("file_type", None) else 0
    )
